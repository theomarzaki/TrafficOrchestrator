...
featuresTrain = torch.zeros(math.ceil(train_data.shape[0]/2),2,19)
targetsTrain = torch.zeros(math.ceil(train_data.shape[0]/2),19)

featuresTest = torch.zeros(math.ceil(test_data.shape[0]/2),2,19)
targetsTest= torch.zeros(math.ceil(test_data.shape[0]/2),19)

batch = torch.zeros(1,19)
for idx in range(train_data.shape[0]):
    if idx % 2 != 0:
        batch[0]= torch.Tensor(train_data[idx])
    else:
        pos = math.ceil(idx/2)
        featuresTrain[pos] = batch
        targetsTrain[pos] = torch.Tensor(train_data[idx])
        batch = torch.zeros(1,19)
...
for idx in range(test_data.shape[0]):
    if idx % 2 != 0:
        batch[0]= torch.Tensor(test_data[idx])
    else:
        pos = math.ceil(idx/2)
        featuresTest[pos] = batch
        targetsTest[pos] = torch.Tensor(test_data[idx])
        batch = torch.zeros(1,19)
...
with torch.no_grad():
    ...
    for locations, labels in test_loader:
        ...
        for idx,actual in enumerate(locations):
            if((actual.numpy() == predicted.numpy()[idx]).all()):
                correct += 1
