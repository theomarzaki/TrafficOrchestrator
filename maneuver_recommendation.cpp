...
private:
	...
	vector<Waypoint*> waypoints; // A list of waypoints indicating a target position, speed, and lane for the vehicle.
...
public:
	...
	ManeuverRecommendation(tuple<uint8_t, uint8_t, uint8_t> version, uint64_t timestamp, string uuid_vehicle, string uuid_to, string uuid_maneuver,
	vector<Waypoint*> waypoints, uint64_t timestamp_action, uint32_t latitude_action, uint32_t longitude_action,
	uint16_t speed_action, uint4 lane_position, string signature) :
	type(type),
	context(context),
	origin(origin),
	timestamp(timestamp),
...
void setWaypoints(vector<Waypoint*>);
...
ManeuverRecommendation::~ManeuverRecommendation() {}
...
vector<Waypoint*> ManeuverRecommendation::getWaypoints(){return waypoints;}
...
void ManeuverRecommendation::setWaypoints(vector<Waypoint*> parameter){waypoints = parameter;}
...
std::ostream& operator<<(std::ostream& os, ManeuverRecommendation * maneuverRec) {
  ...
  << "]\n";
  for(Waypoint * w : maneuverRec->getWaypoints()) {
  	os << w;
  }
